...
void createTextureSamplers(VkPhysicalDevice &physicalDevice, VkDevice &device, std::vector<VkSampler> & textureSamplers, int numberOfTextures){
...
void updateTextureImageViewsAdd(VkDevice &device, VkImage &textureImage, uint32_t &mipLevels,
                                std::vector<VkImageView> &textureImageViews){
    textureImageViews.push_back(createImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT,
                                                mipLevels, device));
}

void createTextureImageViews(VkDevice &device, std::vector<VkImage> &textureImages, uint32_t &mipLevels,
                             std::vector<VkImageView> &textureImageViews){
...
void transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout,
                           uint32_t mipLevels, VkCommandPool &commandPool, VkDevice &device, VkQueue &graphicsQueue) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands(commandPool, device);
...
void createImages(uint32_t width, uint32_t height, uint32_t mipLevels, uint32_t arrayNumber, VkSampleCountFlagBits numSamples,
    ...
    imageInfo.extent.depth = 1;
    imageInfo.mipLevels = mipLevels;
    imageInfo.arrayLayers = arrayNumber;
    imageInfo.format = format;
    imageInfo.tiling = tiling;
    imageInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
    imageInfo.usage = usage;
...
void createTextureImagesSB(uint32_t &mipLevels, VkDevice &device, VkPhysicalDevice &physicalDevice,
    ...
    for (const auto& face : texturePaths) {
        pixels = stbi_load(face.c_str(), &width, &height, &channels, STBI_rgb_alpha);
        if (!pixels) {
    ...
    createImages(width, height, mipLevels, texturePaths.size(), VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_UNORM,
    ...
    copyBufferToImage(stagingBuffer, textureImage, static_cast<uint32_t>(width),
                      static_cast<uint32_t>(height), commandPool, device, graphicsQueue);
    //transitioned to VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL while generating mipmaps

    vkDestroyBuffer(device, stagingBuffer, nullptr);
    vkFreeMemory(device, stagingBufferMemory, nullptr);

    generateMipmaps(textureImage, VK_FORMAT_R8G8B8A8_SRGB, width, height, mipLevels, physicalDevice,
                    device, commandPool, graphicsQueue);
}

void createTextureImages(uint32_t &mipLevels, VkDevice &device, VkPhysicalDevice &physicalDevice,
                         VkCommandPool &commandPool, VkQueue &graphicsQueue, std::vector<VkImage> &textureImages,
                         std::vector<VkDeviceMemory> &textureImageMemorys, std::vector<std::string> texturePaths){
    for (int i = 0; i < texturePaths.size(); ++i) {
        int texWidth, texHeight, texChannels;
        std::string texturePath = "textures/" + texturePaths[i];
...
/*
void updateTextureImagesDelete(uint32_t &mipLevels, VkDevice &device, VkPhysicalDevice &physicalDevice,
                            VkCommandPool &commandPool, VkQueue &graphicsQueue, std::vector<VkImage> &textureImages,
                            std::vector<VkDeviceMemory> &textureImageMemorys, std::string newTexturePath){
...
void createTextureImage(uint32_t &mipLevels, VkDevice &device, VkPhysicalDevice &physicalDevice,
                        VkCommandPool &commandPool, VkQueue &graphicsQueue, VkImage &textureImage,
                        VkDeviceMemory &textureImageMemory, std::string imagePath) {
    int texWidth, texHeight, texChannels;
    std::string texturePath = imagePath;
    std::string texturePathWaterMelon = "fonts/tata_0.png";
...
void loadFontTexture(VkDevice& device, VkPhysicalDevice& physicalDevice, VkCommandPool& commandPool, VkQueue& graphicsQueue,
                     VkImage& textureImage, VkDeviceMemory& textureImageMemory, VkImageView& textureImageView, VkSampler& textureSampler) {
    int texWidth, texHeight, texChannels;
    stbi_uc* pixels = stbi_load("fonts/tata_0.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
    VkDeviceSize imageSize = texWidth * texHeight * 4;
    ...
    transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, commandPool, device, graphicsQueue);
    ...
    transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 1, commandPool, device, graphicsQueue);

    vkDestroyBuffer(device, stagingBuffer, nullptr);
    vkFreeMemory(device, stagingBufferMemory, nullptr);

    textureImageView = createImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, 1, device);
